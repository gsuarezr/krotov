...
def independiente_transformado(c34,c33,c44,c3,c4):
...
def integral_aux(a1,a2,a12,a11,a22,c33,c34,c44,c3,c4,V,B,R,S):
    ...
    independientes=np.exp(independiente_transformado(c34,c33,c44,c3,c4))*np.exp(independiente(V,B,R,S))
    det1=(np.linalg.det(np.array(V)+(1/2)*np.eye(4)))**(1/4)
    det2=(np.linalg.det(np.array(B)+(1/2)*np.eye(4)))**(1/4)
    cons=(1/2*np.pi)**4 *(1/np.pi)**2
    return(independientes*output*cons)/(det1*det2)
...
def integral(V,B,R,S):
    ...
    c3v=lineal3(V,B,R,S)
    c4v=lineal4(V,B,R,S)
    return  integral_aux(a,b,c,d,e,f,g,h,c3v,c4v,V,B,R,S)
